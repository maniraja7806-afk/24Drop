            ...
            <div key={att.id} className="relative inline-flex flex-col group items-center">
              ...
              {!isSending && (
                <button 
                  ...
                  className="absolute -top-1 -right-1 w-5 h-5 bg-neutral-800 text-white rounded-full flex items-center justify-center border border-white/10 hover:bg-neutral-700 transition-colors shadow-lg z-10 md:opacity-0 md:group-hover:opacity-100 active:scale-90"
                >
                  <X className="w-3 h-3" />
                </button>
              )}
...
code = code.replace(/      \{file && \([\s\S]*?      \}\)/, newAttachmentsUI);
